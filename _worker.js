...
async function handleVLESS(request, userID, proxyIP, server) {
    const readableStream = handleWebSocket(request, server);
    const ref = { value: null };
    for await (const chunk0 of readableStream) {
        if (!ref.value) {
            const { version, isUDP, port, hostname, byteOffset } = handleHeader(chunk0, userID);
            if (isUDP && port !== 53) {
...
function handleWebSocket(request, webSocket) {
    webSocket.accept();
...
function handleHeader(buffer, userID) {
    let byteOffset = 0;
    const [version] = new Uint8Array(buffer, byteOffset, 1);
    byteOffset += 1;
    const uuid = new Uint8Array(buffer, byteOffset, 16).reduce((previousValue, currentValue, currentIndex) => previousValue +
    ...
    if (af === 1) {
        ...
        byteOffset += 1;
        hostname = new TextDecoder().decode(buffer.slice(byteOffset, byteOffset + addressLen));
    }
    else if (af === 3) {
        // IPv6
        addressLen = 16;
        const view = new DataView(buffer.slice(byteOffset, byteOffset + addressLen));
        const ipv6 = [];
        for (let i = 0; i < 8; i += 1) {
...
async function handleSocket(server, header, chunk, hosts, ref) {
    ref.value = connect(hosts[0]);
    await writeSocket(ref.value, chunk);
    let sent = false;
    let received = false;
    await ref.value.readable.pipeTo(new WritableStream({
        async write(chunk) {
            received = true;
            if (!sent) {
...
async function writeSocket(socket, chunk) {
